...
static int test_raw_sock(char *addr) {
    ...
    // int flag = 1;
    // if (setsockopt(raw_fd, IPPROTO_ICMP, IP_HDRINCL, &flag, sizeof(flag)) < 0) {
    ...
    servaddr.sin_addr.s_addr = inet_addr("192.168.1.5");  // 192.168.21.37");
    servaddr.sin_port = 0;
    ...
    // unsigned char s_buf[] = {0x45, 0x00, 0x00, 0x54, 0x7c, 0x3a, 0x00, 0x00, 0x40, 0x01, 0xbe, 0x7c, 0xc0, 0xa8,
    ...
    unsigned char iph_buf[] = {0x45, 0x00, 0x00, 0x54, 0x7c, 0x3a, 0x00, 0x00, 0x40, 0x01,
                               0xbe, 0x7c, 0xc0, 0xa8, 0x01, 0x05, 0xb6, 0x3d, 0xc8, 0x07};

    unsigned char icmp_buf[] = {0x08, 0x00, 0x2c, 0x2d, 0x1e, 0x57, 0x00, 0x00, 0x63, 0xb6, 0xa2, 0x35, 0x00,
                                0x01, 0xbc, 0x8b, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11,
    ...
    struct ip *ip = (struct ip *)iph_buf;
    struct ip iph;
    iph.ip_hl = ip->ip_hl;
    iph.ip_v = ip->ip_v;
    ...
    // iph.ip_sum = 0;
    inet_pton(AF_INET, "192.168.1.5", &iph.ip_src);
    inet_pton(AF_INET, "182.61.200.7", &iph.ip_dst);

    // Ip checksum
    iph.ip_sum = 0;
    // iph.ip_sum = ip_checksum((unsigned short *)ip, 10);
    iph.ip_sum = ip_checksum((unsigned short *)&iph, 10);
    memcpy(buf, &iph, 20);
    memcpy(buf, icmp_buf, 64);

    int s_bytes = sendto(raw_fd, buf, sizeof(buf), 0, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
    if (s_bytes < 0) {
        perror("sendto error");
        return -1;
    }
...
int main(int argc, char *argv[]) {
    skt_raw_sock_t *raw_sock = skt_raw_sock_new("192.168.3.26");
    assert(raw_sock);
    ...
    int rt = skt_raw_sock_send(raw_sock, icmp_packet, sizeof(icmp_packet), NULL, "192.168.3.26");
    assert(rt > 0);
