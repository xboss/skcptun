...
static int start_tun_server(struct ev_loop *loop, skt_config_t *conf) {
    ...
    if (skt_server_init(conf->skcp_conf[0], loop, conf->tun_ip, conf->tun_mask) != 0) {
        return -1;
    }
...
static int start_tun_client(struct ev_loop *loop, skt_config_t *conf) {
    ...
    if (skt_client_init(conf->skcp_conf, conf->skcp_conf_cnt, loop, conf->tun_ip, conf->tun_mask) != 0) {
        return -1;
    }
...
int main(int argc, char *argv[]) {
    ...
    int rt = 0;
    SKT_IF_TUN_SERV_MODE(conf->mode) { rt = start_tun_server(loop, conf); }

    SKT_IF_TUN_CLI_MODE(conf->mode) { rt = start_tun_client(loop, conf); }
