...
//             LOG_E("skt.cb is not table %s %d %d", _v_fn_name, lua_type(g_ctx->L, -1), lua_gettop(g_ctx->L)); \
//             _m_rt_value = 1;                                                                                 \
//             lua_pop(g_ctx->L, 2);                                                                            \
//             break;                                                                                           \
//         }                                                                                                    \
...
// static int on_tcp_server_accept(int fd) {
//     SKT_LUA_PUSH_CALLBACK_FUN("on_tcp_server_accept") return 1;

//     lua_pushinteger(g_ctx->L, fd);          // 自动弹出
//     int rt = lua_pcall(g_ctx->L, 1, 0, 0);  // 调用函数，调用完成以后，会将返回值压入栈中
//     if (rt) {
//         LOG_E("%s", lua_tostring(g_ctx->L, -1));
//         lua_pop(g_ctx->L, 3);
//         return 1;
//     }
//     lua_pop(g_ctx->L, 2);
//     return 0;
// }
// static void on_tcp_server_recv(int fd, char *buf, int len) {
//     SKT_LUA_PUSH_CALLBACK_FUN("on_tcp_server_recv") return;
//     lua_pushinteger(g_ctx->L, fd);          // 自动弹出
...
//     lua_pop(g_ctx->L, 2);
// }
// static void on_tcp_server_close(int fd) {
//     // LOG_I("lua stack top: %d", lua_gettop(g_ctx->L));
//     SKT_LUA_PUSH_CALLBACK_FUN("on_tcp_server_close") return;
...
// static void on_tcp_client_recv(int fd, char *buf, int len) {
//     SKT_LUA_PUSH_CALLBACK_FUN("on_tcp_client_recv") return;
//     lua_pushinteger(g_ctx->L, fd);          // 自动弹出
//     lua_pushlstring(g_ctx->L, buf, len);    // 自动弹出
//     int rt = lua_pcall(g_ctx->L, 2, 0, 0);  // 调用函数，调用完成以后，会将返回值压入栈中
//     if (rt) {
...
//     lua_pop(g_ctx->L, 2);
// }
// static void on_tcp_client_close(int fd) {
//     SKT_LUA_PUSH_CALLBACK_FUN("on_tcp_client_close") return;
...
// static void on_skcp_server_accept(skcp_t *skcp, uint32_t cid) {
//     SKT_LUA_PUSH_CALLBACK_FUN("on_skcp_server_accept") return;
...
// static void on_skcp_server_recv_data(skcp_t *skcp, uint32_t cid, char *buf, int len) {
//     SKT_LUA_PUSH_CALLBACK_FUN("on_skcp_server_recv_data") return;
//     lua_pushlightuserdata(g_ctx->L, skcp);  // 自动弹出
//     lua_pushinteger(g_ctx->L, cid);         // 自动弹出
//     lua_pushlstring(g_ctx->L, buf, len);    // 自动弹出
...
//     lua_pop(g_ctx->L, 2);
// }
// static void on_skcp_server_close(skcp_t *skcp, uint32_t cid) {
//     SKT_LUA_PUSH_CALLBACK_FUN("on_skcp_server_close") return;
...
// static int on_skcp_server_check_ticket(skcp_t *skcp, char *ticket, int len) {
//     SKT_LUA_PUSH_CALLBACK_FUN("on_skcp_server_check_ticket") return -1;
//     lua_pushlightuserdata(g_ctx->L, skcp);   // 自动弹出
...
//     lua_pop(g_ctx->L, 2);  // 弹出 skt 和 cb
//     if (!isnum) {
//         LOG_E("return value is not integer, when call on_skcp_server_check_ticket in lua");
//         lua_pop(g_ctx->L, 1);  // 弹出返回值
//         return -1;
//     }

//     lua_pop(g_ctx->L, 1);  // 弹出返回值
//     // LOG_I("lua stack on_skcp_server_check_ticket top: %d", lua_gettop(g_ctx->L));
//     return ret;
// }

// static void on_skcp_client_recv_cid(skcp_t *skcp, uint32_t cid) {
//     // LOG_I("on_skcp_recv_cid cid: %u", cid);

//     SKT_LUA_PUSH_CALLBACK_FUN("on_skcp_client_recv_cid") return;

//     lua_pushlightuserdata(g_ctx->L, skcp);  // 自动弹出
//     lua_pushinteger(g_ctx->L, cid);         // 自动弹出
//     int rt = lua_pcall(g_ctx->L, 2, 0, 0);  // 调用函数，调用完成以后，会将返回值压入栈中
//     if (rt) {
//         LOG_E("%s, when call on_skcp_client_recv_cid in lua", lua_tostring(g_ctx->L, -1));
//         lua_pop(g_ctx->L, 3);
//         return;
//     }
//     lua_pop(g_ctx->L, 2);
// }
// static void on_skcp_client_recv_data(skcp_t *skcp, uint32_t cid, char *buf, int len) {
//     SKT_LUA_PUSH_CALLBACK_FUN("on_skcp_client_recv_data") return;
//     lua_pushlightuserdata(g_ctx->L, skcp);  // 自动弹出
//     lua_pushinteger(g_ctx->L, cid);         // 自动弹出
//     lua_pushlstring(g_ctx->L, buf, len);    // 自动弹出
//     int rt = lua_pcall(g_ctx->L, 3, 0, 0);  // 调用函数，调用完成以后，会将返回值压入栈中
//     if (rt) {
//         LOG_E("%s, when call on_skcp_client_recv_data in lua", lua_tostring(g_ctx->L, -1));
//         lua_pop(g_ctx->L, 3);
//         return;
//     }
//     lua_pop(g_ctx->L, 2);
// }
// static void on_skcp_client_close(skcp_t *skcp, uint32_t cid) {
//     SKT_LUA_PUSH_CALLBACK_FUN("on_skcp_client_close") return;

//     lua_pushlightuserdata(g_ctx->L, skcp);  // 自动弹出
//     lua_pushinteger(g_ctx->L, cid);         // 自动弹出
//     int rt = lua_pcall(g_ctx->L, 2, 0, 0);  // 调用函数，调用完成以后，会将返回值压入栈中
//     if (rt) {
//         LOG_E("%s, when call on_skcp_client_close in lua", lua_tostring(g_ctx->L, -1));
//         lua_pop(g_ctx->L, 3);
//         return;
//     }
//     lua_pop(g_ctx->L, 2);
...
//     //     return -1;
//     // }
//     // LOG_I("lua stack skt top: %d", lua_gettop(g_ctx->L));

//     if (conf->skcp_serv_conf_list && conf->skcp_serv_conf_list_size > 0) {
//         // skcp server
//         for (size_t i = 0; i < conf->skcp_serv_conf_list_size; i++) {
//             conf->skcp_serv_conf_list[i]->on_accept = on_skcp_server_accept;
//             conf->skcp_serv_conf_list[i]->on_check_ticket = on_skcp_server_check_ticket;
//             conf->skcp_serv_conf_list[i]->on_close = on_skcp_server_close;
//             conf->skcp_serv_conf_list[i]->on_recv_data = on_skcp_server_recv_data;
//         }
//     }
...
//             conf->skcp_cli_conf_list[i]->on_close = on_skcp_client_close;
//             conf->skcp_cli_conf_list[i]->on_recv_cid = on_skcp_client_recv_cid;
//             conf->skcp_cli_conf_list[i]->on_recv_data = on_skcp_client_recv_data;
//         }
//     }
...
//             conf->etcp_serv_conf_list[i]->on_accept = on_tcp_server_accept;
//             conf->etcp_serv_conf_list[i]->on_recv = on_tcp_server_recv;
//             conf->etcp_serv_conf_list[i]->on_close = on_tcp_server_close;
//         }
//     }
...
//             conf->etcp_cli_conf_list[i]->on_recv = on_tcp_client_recv;
//             conf->etcp_cli_conf_list[i]->on_close = on_tcp_client_close;
//         }
//     }
